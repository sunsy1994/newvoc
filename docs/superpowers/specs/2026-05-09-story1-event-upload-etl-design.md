...
| 中文名 | 字段名 | 必填 | 示例 | 说明 |
| --- | --- | --- | --- | --- |
| 原始事件ID | `raw_event_id` | 选填 | `EVT-2026-001` | 你可以自己传稳定事件ID；不传时 ETL 根据事件名称等字段生成系统 `event_id`。 |
...
| 所属事件ID | `event_id` | 必填 | `EVT-2026-001` | 对应事件表生成后的 `event_id`。第一版由使用者明确归属事件。 |
...
- 第一版内容上传时明确给 `event_id`。
- 标准化 ETL 根据 `dwd_content.event_id` 自动生成 `rel_event_content`。
- `match_type = manual`。
- `match_score = 1`。
...
- 上传模板只反推当前 `data_access_schema.sql` 已存在的 ODS 字段；自查时为了解决评论归属过度依赖系统ID的问题，已补充 `ods_comment_upload.content_source_url`。
