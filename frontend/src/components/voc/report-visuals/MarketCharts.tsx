...
export function M4ChannelEfficiency({ chart }: ChartProps) {
  ...
  if (rows.length === 1) {
    const row = rows[0];
    return <ReportVisualShell chart={chart} hasData><div aria-label="渠道效率图" role="img" className="flex min-h-56 items-center justify-center"><div className="rounded-full border border-[var(--theme-border)] bg-[var(--theme-selected-bg)] px-10 py-8 text-center"><p className="font-semibold text-[var(--theme-ink)]">{text(row.platform)}</p><p className="mt-2 text-sm text-[var(--theme-muted)]">{compact(num(row.total_volume))} 声量 · {compact(num(row.engagement_per_content))} 单内容互动</p></div></div></ReportVisualShell>;
  }
  const maxX = Math.max(...rows.map((row) => num(row.total_volume)), 1);
  const maxY = Math.max(...rows.map((row) => num(row.engagement_per_content)), 1);
  return <ReportVisualShell chart={chart} hasData={rows.length > 0}><svg aria-label="渠道效率图" role="img" viewBox="0 0 760 300" className="h-auto w-full">
